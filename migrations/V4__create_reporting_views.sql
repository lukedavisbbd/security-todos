...
SELECT 
    ...
    COUNT(CASE WHEN s.status_name ILIKE ANY (ARRAY['closed', 'done', 'completed']) THEN 1 END) AS completed_tasks,
    COUNT(CASE WHEN s.status_name ILIKE ANY (ARRAY['in-progress', 'todo', 'open']) THEN 1 END) AS active_tasks,
    COALESCE(
        AVG(
            CASE 
                WHEN latest_history.timestamp IS NOT NULL
...
SELECT 
    ...
    COUNT(DISTINCT CASE WHEN s.status_name ILIKE ANY (ARRAY['closed', 'done', 'completed']) THEN t.task_id END) AS completed_tasks,
    COUNT(DISTINCT CASE WHEN s.status_name ILIKE ANY (ARRAY['in-progress', 'todo', 'open']) THEN t.task_id END) AS active_tasks,
    COUNT(DISTINCT CASE WHEN t.assigned_to_id IS NULL THEN t.task_id END) AS unassigned_tasks,
    COALESCE(
        ROUND(
